...
<form method="post" enctype="multipart/form-data">
	...
	<div class="row">
		<div class="col-md-6">
			...
			<?php if($aUsr['role'] == DISTRIBUTOR_ROLE) {?>
				<input type="hidden" name="val[warehouse_id]" value="<?php echo $aUsr['warehouse_id']; ?>">
			<?php } elseif($bEdit){?>
        <input type="hidden" name="val[warehouse_id]" value="<?php echo $aRow['warehouse_id'];?>">
      <?php }else{ ?>
			<div class="form-group">
				<label>Warehouse</label>
				<select required="" name="val[warehouse_id]" class="form-control border-input">
    ...
    <?php } ?>
			...
			<?php }else{?>
				<input type="hidden"  name="val[email]" value="<?php set_default_value('val[email]', $aRow ? $aRow['email'] : ''); ?>" >
			<?php }?>
			<div class="form-group">
